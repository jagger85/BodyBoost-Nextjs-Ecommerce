...
import { DEFAULT_PAYMENT_MEHTOD, PAYMENT_METHODS } from '@/lib/constants'
...
const PaymentMethodForm = ({ preferredPaymentMethod }: { preferredPaymentMethod: string | null }) => {
  ...
  const form = useForm<z.infer<typeof paymentMethodSchema>>({
    ...
    defaultValues: {
      type: preferredPaymentMethod || DEFAULT_PAYMENT_MEHTOD,
    },
  })
                    ...
                    <FormControl>
                      <RadioGroup onValueChange={field.onChange} className='flex flex-col space-y-2'>
                        {PAYMENT_METHODS.map(paymentMethod => (
                          <FormItem key={paymentMethod} className='flex item space-x-3 space-y-0' >
                          <FormControl>
                            <RadioGroupItem value={paymentMethod} checked={field.value === paymentMethod}/>
                          </FormControl>
                          <FormLabel className='font-normail'>
                            {paymentMethod}
                          </FormLabel>
                          </FormItem>
                        ))}
